...
def start(board: Board, agent: Agent):
    ...
    while(not agent_done):
        ...
        '''
        THIS IS WHERE WE NEED TO ADD THE AI PORTION, WE NEED TO CHOOSE THE NEW TILES TO FLAG/OPEN 
        CANNOT JUST CHOOSE RANDOMLY BUT NEED TO INFER
        1. DOUBLE INFERENCE -> use 2 equations instead of just 1
        2. SUBSET REDUCTION -> reduce an equation to lesser variables if possible -> A+B+C+D+E+F+G=3, D+E+F+G = 1 -> A+B+C = 2
        3. CONSTRAINT SATIS

        how are we gonna represent our data/ knowledge base?
...
def check_neighbors(curr_tile: BoardTile, board: Board, unfinished_tiles: list, tiles_to_open: list, score: int):
    ...
    mine_flagged_neighbors = [tile for tile in neighbors if tile.is_flagged or (tile.is_opened and tile.type == TILES.MINE)]
